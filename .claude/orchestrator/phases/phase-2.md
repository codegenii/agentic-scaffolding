...
- **REQUEST CHANGES**: re-invoke `spec-writer` in revise mode with the blocking findings quoted verbatim, commit (`spec(<slug>): address spec-review`), re-invoke spec-reviewer. Cap 3 rounds, then escalate.
...
- Feedback: revise through `spec-writer` (revise mode, feedback quoted verbatim), re-run spec-reviewer (counts against the same cap of 3), re-present.
