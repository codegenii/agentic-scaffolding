...
**Exit gate:**
- The test-writer report shows `Result: OK`, `Blockers: None.`
- `git diff --stat` shows only test files (matching `${TEST_GLOB}`) under `<unit>`, and agrees with the report's `Files touched`.
- `${TEST_SCOPE_CMD}` (unit suite) exits non-zero — your own run, not the report's `Commands` line.
