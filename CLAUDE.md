...
- Permissions: a worktree created by a plain `git worktree add` lacks the main checkout's `.claude/settings.local.json`. Run `./scripts/setup-worktree.sh` from inside the new worktree to copy or merge those local permission grants (`/new-chore` and `/new-feature` do this automatically).
