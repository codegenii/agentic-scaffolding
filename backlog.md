...
> Phases 4–7 in `.claude/orchestrator/phases/` each repeat the same brief skeleton: the "Spec path (reference only — do not read)" line, the context-card line, and the `## Extracted <Section>` blocks. Extract a single brief template (e.g. in `orchestrator.md`) that each phase fills with its agent, instruction, and section list, so each phase file states only what is unique to it. Two properties of the current briefs must survive the refactor: (a) cache-prefix ordering — stable content first, volatile content (previous failures, review findings) in a trailing section, with the byte-identical-prefix notes in phases 6–7 kept true; (b) markdownlint-clean formatting — backticked `<placeholder>` tokens and lone-`>` blank lines around in-quote headings. Preserve exact brief semantics — the workers' behavior must not change.
...
## 9. Repo-wide markdownlint pass + config

> Phases 6–7 are markdownlint-clean (2026-07-11); the rest of the template still carries the original violations: bare `<placeholder>` tokens in phases 1/4/5 brief templates (MD033), unlanguaged fences in CONTRIBUTING and bootstrap-guide (MD040), compact table pipes repo-wide (MD060), and missing blank lines around lists/headings in several files. Sweep all markdown to the conventions in phases 6–7, and add a `.markdownlint.jsonc` that (a) disables MD024 or scopes it — the duplicated `## Extracted ...` brief headings are a load-bearing contract the agents match on, never rename them — and (b) records the chosen table style. Content-neutral: no wording changes, only formatting.
